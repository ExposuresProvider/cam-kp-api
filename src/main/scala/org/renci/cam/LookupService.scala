...
object LookupService extends LazyLogging {
   ...
   *
   * This should really be a TRAPI object, but since there isn't a TRAPI object that includes both a node
   * and an edge, we make one up here.
   *
   * @param subj The subject of this relation.
   * @param preds The predicates connecting this subject to this object.
   * @param biolinkPredicates The predicates in `pred` translated into Biolink predicates. The predicate IRIs are used
   *                          as the keys.
   * @param obj The object of this relation.
   * @param objRelations The relations connected to the object of this relation. CAM-KP is designed so that this
   *    list is not necessary, since `subj` should be connected to every other entity in its graph (even if only by a
   *    `biolink:related_to` predicate); however, it is intended to test whether this is in fact the case.
   * @param g The graph that this relation comes from.
   */
  case class Relation(
    subj: LabeledIRI,
   ...
   *
   * @param queryId The identifier queried with lookup.
   * @param normalizedId The identifier actually used for the query. This will be identical to queryId if normalization
   *                     was not requested or
   * @param biolinkPredicates All predicates connected to the identifier being looked up.
   * @param relations All relations connected to the identifier being looked up.
   * @param subjectTriples All subject triples for the identifier being looked up (i.e. triples in the triplestore
   *                       in the shape `normalizedId ?p ?o`).
   * @param objectTriples All object triples for the identifier being looked up (i.e. triples in the triplestore
   *                      in the shape `?s ?p normalizedId`)
   */
  case class Result(
                     queryId: String,
   ...
   *
   * @param iri An IRI.
   * @param label An optional label for this IRI.
   */
   ...
   *
   * This function is intended to be called recursively: it will call itself to get all the relations for every subject
   * of a relation.
   *
   * @param subject The subject IRI to find relations for.
   * @param hopLimit The number of hops to find relations for. If equal to zero, this will return an empty Seq.
   * @param subjectsPreviouslyQueried Subject IRIs that have already been queried. We will not recurse into these subject IRIs.
   * @return A ZIO that returns a Relation, possibly containing multiple Relations in its objRelations field.
   */
  def getRelations(subject: String, hopLimit: Int, subjectsPreviouslyQueried: Set[String]): ZIO[ZConfig[AppConfig] with HttpClient with Has[BiolinkData], Throwable, Iterable[Relation]] = for {
    ...
    relations = objectMap map { case (obj, results) =>
      ...
      val objLabeled = results.map({ res =>
      ...
      val subjLabeled = results.flatMap({ res =>
   ...
   *
   * @param subject The IRI of a subject to investigate.
   * @param nodeNormURL The URL of NodeNorm to call for normalization (should end with '/get_normalized_nodes'),
   *                    or None if no normalization is required.
   * @param hopLimit The maximum number of hops to follow.
   * @return A ZIO returning a Result to the user.
   */
  ...
  def lookupRouteR(lookupEndpoint: LookupEndpoint): HttpRoutes[RIO[EndpointEnv, *]] =
    ZHttp4sServerInterpreter[EndpointEnv]()
      .from(lookupEndpoint) { case (subj, nodeNormURL, hopLimit) => lookup(subj, nodeNormURL, hopLimit)
