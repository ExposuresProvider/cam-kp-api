...
object BlazegraphTest extends DefaultRunnableSpec {
  ...
  val suite1 = suite("BlazegraphTestSpec")(
    testM("bindings") {
      ...
      val uri = uri"http://152.54.9.207:9999/blazegraph/sparql"
          .withQueryParam("query", query)
          .withQueryParam("format", "json")
      val request = Request[Task](Method.POST, uri).withHeaders(Accept(MediaType.application.json), `Content-Type`(MediaType.application.json))
      for {
