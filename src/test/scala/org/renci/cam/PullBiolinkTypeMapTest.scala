    ...
    suite("PullBiolinkTypeMapTestSpec")(
      testM("pull") {
        for {
          ...
          filteredJson <- Task.effect(contextJson.deepDropNullValues.mapObject(f => f.filter(pred => pred._2.isString && pred._1 != "type" && pred._1 != "id" && pred._1 != "@vocab" )))
          _ = Files.writeString(Paths.get("src/main/resources/prefixes.json"), filteredJson.toString(), StandardOpenOption.CREATE)
